...
public class Service {
    private static DatabaseConnection dbConnection = new H2DatabaseConnection();
    ...
    public static void removeDepartment(Department d) {
        try (Connection con = dbConnection.getConnection()) {
            PreparedStatement stm1 = con.prepareStatement("DELETE FROM Employee WHERE DepartmentID=?");
            stm1.setInt(1, d.departmentID);
            stm1.executeUpdate();

            PreparedStatement stm2 = con.prepareStatement("DELETE FROM Department WHERE ID=?");
